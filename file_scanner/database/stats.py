...
class StatsManager(DatabaseManager):
    ...
    def _update_schema(self) -> None:
        ...
        # Add total_size_bytes to file_types if it doesn't exist
        file_types_columns = self._get_table_columns("file_types")
        
        if "total_size_bytes" not in file_types_columns:
            self._add_column(
                "file_types",
                "total_size_bytes",
                "INTEGER DEFAULT 0"
    ...
    def save_scan_results(self, scan_result: ScanResult) -> int:
        ...
        Args:
            scan_result: Results of the directory scan
            
        Returns:
            ID of the new scan record
        """
