...
def create_file_table(files: List[Dict], console: Console) -> Table:
    ...
    for file in files:
        table.add_row(
            f"{'.' if file['is_hidden'] else ''}{file['file_name']}",
            str(file['relative_path']),
            file['extension'] or "(none)",
            format_size(file['size_bytes']),
            format_timestamp(datetime.fromisoformat(file['modified_date']))
        )
...
def create_directory_tree(
    ...
    for dir_info in sorted_dirs:
        parent_path = Path(dir_info['relative_path']).parent
        
        if parent_path in path_to_tree:
            branch = path_to_tree[parent_path].add(
                f"[bold blue]{Path(dir_info['relative_path']).name}/[/]"
            )
            path_to_tree[Path(dir_info['relative_path'])] = branch
...
def create_scan_header(scan_result: ScanResult) -> List[str]:
    ...
    return [
        ...
        f"[bold]Total Size:[/] [green]{scan_result.formatted_total_size}[/]",
